/**
 * 
 * Original code was javascript from a dependancy...
 * 
 * 
 */

import {DeserializationLibrary} from "../state/Deserialize.js";
...
type TypedArrayConstructor = Int32ArrayConstructor | Float32ArrayConstructor;

export class MinQueueHeap<T> {
	...
	constructor(capacity = 64, PrioritiesBackingArrayType: TypedArrayConstructor = Float32Array) {

		this._capacity = capacity;
		this._keys = new Array<T>(this.capacity + ROOT_INDEX);
...
DeserializationLibrary.register(MinQueueHeap);


/**
 * 
 * MinQueueHeap which is /slightly/ faster for number types, you must provide the number (key) type though!
 * 
 */
export class MinQueueHeapNumber {
	private readonly _capacity: number;
	...
	constructor(capacity = 64, KeyArrayType: TypedArrayConstructor = Int32Array, PrioritiesBackingArrayType: TypedArrayConstructor = Float32Array) {

		this._capacity = capacity;
		this._keys = new KeyArrayType(capacity + ROOT_INDEX);
		this._priorities = new PrioritiesBackingArrayType(capacity + ROOT_INDEX);
		// to keep track of whether the first element is a deleted one
		this._hasPoppedElement = false;
...
DeserializationLibrary.register(MinQueueHeapNumber);